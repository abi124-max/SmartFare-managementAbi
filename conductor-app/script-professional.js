...
function updateValidationSummary(data) {
    ...
    // Don't update validated tickets here - it comes from backend
    updateStatsDisplay(totalPassengers, null, totalDiscrepancies, analytics.todayRevenue);
}
...
async function loadCheckedTicketsCount() {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/bookings/stats/checked`);
        
        if (response.ok) {
            const stats = await response.json();
            console.log('Checked tickets stats:', stats);
            
            // Update the validated tickets count with today's checked tickets
            document.getElementById('validatedTickets').textContent = stats.todayChecked || 0;
            
            // Store for analytics
            analytics.ticketsScanned = stats.todayChecked || 0;
            
        } else {
            console.warn('Failed to load checked tickets count');
        }
    } catch (error) {
...
async function validateScannedTicket() {
    ...
    try {
        ...
        const bookingRef = scannedTicketData.bookingId;
        ...
        const API_BASE_URL = 'http://localhost:8081/api';
        const response = await fetch(`${API_BASE_URL}/bookings/${bookingRef}/verify`, {
            method: 'POST',
            headers: {
        ...
        if (result.success) {
            ...
            // Reload checked tickets count
            await loadCheckedTicketsCount();
            
            // Also update Firebase for the destination stop
